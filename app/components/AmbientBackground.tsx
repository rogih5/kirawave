    ...
    useEffect(() => {
        ...
        if (ambKey === 'cafe') {
            ...
            const draw = () => {
                ...
                puffs.forEach(p => {
                    p.y += p.vy;
                    p.x += p.vx + Math.sin(t * 0.02 + p.phase) * 0.3;
                    p.r += 0.08;
        ...
        if (ambKey === 'forest') {
            ...
            const draw = () => {
                ...
                flies.forEach(f => {
                    ...
                    f.y += f.vy;
                    f.x += f.vx + Math.sin(t * 0.025 + f.phase) * 0.5;
                    ...
                    ctx.fillStyle = `rgba(187,247,208,${f.op})`;
                    ctx.fill();
                });
                rafRef.current = requestAnimationFrame(draw);
            };
        ...
        if (ambKey === 'white') {
            ...
            const draw = () => {
                ...
                rings.forEach(ring => {
                    ...
                    ctx.strokeStyle = `rgba(226,232,240,${ring.op})`;
                    ctx.lineWidth = 1.5;
                    ctx.stroke();
                });
                rafRef.current = requestAnimationFrame(draw);
...
export default memo(function AmbientBackground({ ambKey, intensity = 0.6 }: Props) {
    if (Platform.OS !== 'web') return null; // native: sem efeito por ora
    return (
